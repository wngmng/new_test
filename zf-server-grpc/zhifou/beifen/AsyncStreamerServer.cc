...
private:
	...
	std::vector<std::vector<std::string>> question_list_ = get_my_queston(request_.userid());
    
	int count_ = (question_list_.size() == 0)?0: (question_list_[0].size() == 0)?0:question_list_[0].size();
...
private:
	...
	std::vector<std::vector<std::string>> answer_list_ = get_my_answer(request_.userid());
    
	int count_ = (answer_list_.size() == 0)?0: (answer_list_[0].size() == 0)?0:answer_list_[0].size();
...
public:
    ...
    HandlerStatus SendResponse(void* opaque_handle) override 
    {
        ...
        try {
			...
			response.set_createdate(article_list_[3][article_list_[0].size() - count_ - 1]);
...
private:
	...
	std::vector<std::vector<std::string>> article_list_ = get_my_article(request_.userid());
   
	int count_ = (article_list_.size() == 0)?0: (article_list_[0].size() == 0)?0:article_list_[0].size();
...
public:
    ...
    HandlerStatus SendResponse(void* opaque_handle) override 
    {
        ...
        try {
			...
			response.set_username(fans_list_[2][fans_list_[0].size() - count_ - 1]);
...
private:
	...
	std::vector<std::vector<std::string>> fans_list_ = get_my_fans(request_.userid());
    
	int count_ = (fans_list_.size() == 0)?0: (fans_list_[0].size() == 0)?0:fans_list_[0].size();
...
using AsyncStreamHandlerBase5 = grpcserver::GenericAsyncHandler< 
    GRPC_Namespace::User, GRPC_Namespace::Users, 
    grpcserver::AsyncStreamingWriter, T>; 
...
public:
    ...
    HandlerStatus SendResponse(void* opaque_handle) override 
    {
        ...
        try {
			...
			response.set_username(following_list_[2][following_list_[0].size() - count_ - 1]);
...
private:
	...
	std::vector<std::vector<std::string>> following_list_ = get_my_following(request_.userid());
    
	int count_ = (following_list_.size() == 0)?0: (following_list_[0].size() == 0)?0:following_list_[0].size();
...
private:
	...
	std::vector<std::vector<std::string>> question_list_ = get_my_favorite_queston(request_.userid());
...
private:
	...
	std::vector<std::vector<std::string>> article_list_ = get_my_favorite_article(request_.userid());
...
private:
	...
	std::vector<std::vector<std::string>> answer_list_ = get_my_favorite_answer(request_.userid());
    int count_ = (answer_list_.size() == 0)?0: (answer_list_[0].size() == 0)?0:answer_list_[0].size();
